...
defmodule Mix.Tasks.Day do
  ...
  def run(day) do
    if day == ["all"] do
      ...
      {total_time, _} = :timer.tc(fn ->
        input =
          apply(String.to_existing_atom("Elixir.Advent2023.Day#{day}"), :setup, [])

        {p1_time, p1_answer} = :timer.tc(fn ->
          apply(String.to_existing_atom("Elixir.Advent2023.Day#{day}"), :p1, [input])
        end)

        {p2_time, p2_answer} = :timer.tc(fn ->
          apply(String.to_existing_atom("Elixir.Advent2023.Day#{day}"), :p2, [input])
        end)
